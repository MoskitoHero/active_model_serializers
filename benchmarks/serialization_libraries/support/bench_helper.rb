module BenchHelper
  ...
  def seed_data
    ...
    id = 1
    user = User.create!(id: id, first_name: 'Diana', last_name: 'Prince', birthday: anchor_time,
                        created_at: anchor_time, updated_at: anchor_time)
    id += 1

    data_config[:posts].times do
      post = Post.create!(id: id, user_id: user.id, title: 'Some Post', body: 'awesome content',
                          created_at: anchor_time, updated_at: anchor_time)
      id += 1
      data_config[:comments_per_post].times do
        Comment.create!(id: id, author: 'me', comment: 'nice blog', post_id: post.id, created_at: anchor_time,
  ...
  def assert_equal_json(description, expected_json_file, actual_json_document)
    ...
    FileUtils.cp(actual_json_file, expected_json_file) unless File.exist?(expected_json_file)
    ...
    cmd = "diff --side-by-side #{expected_json_file} #{actual_json_file} > #{File.join(temp_dir,
                                                                                       "actual--#{File.basename(expected_json_file)}")}.diff"
    system(cmd)
  ...
  def render_data(data, render_gem)
    ...
    when :barley then render_with_barley(data)
    when :barley_cache then render_with_barley_cache(data)
    else raise ArgumentError, "Cannot render unknown gem '#{render_gem.inspect}'"
    end
  end
  ...
  def render_with_barley(data)
    "#{data.class}BarleySerializer".constantize.new(
      data
       ).as_json
  end

  def render_with_barley_cache(data)
    "#{data.class}BarleySerializer".constantize.new(
      data,
      cache: true
       ).as_json
